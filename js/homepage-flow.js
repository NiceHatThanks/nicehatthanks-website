...
(function () {
  ...
  if (nav) nav.innerHTML = `<a href="#product">What it does</a><a href="#interactive-demo">Try it</a><a href="#story">Why</a><a href="/setup/">Setup</a><a href="https://github.com/NiceHatThanks/scoopy-node" target="_blank" rel="noreferrer">GitHub</a><a href="/shop.html">Shop</a>`;

  const style = document.createElement('style');
  style.textContent = `
    .hero{min-height:68vh;padding-top:72px;padding-bottom:82px}.hero-copy h1{font-size:clamp(2.65rem,5vw,5rem)!important;line-height:1.01!important;max-width:760px}.hero-copy .intro{max-width:650px;margin-top:24px}
    .open-source-strip{display:grid;grid-template-columns:minmax(0,.82fr) minmax(0,1.18fr);gap:clamp(40px,7vw,100px);padding:68px clamp(28px,5vw,64px);margin-bottom:92px;border:1px solid var(--border);border-radius:30px;background:var(--surface)}.open-source-strip h2{font-size:clamp(2rem,3.5vw,3.35rem);line-height:1.04}.open-source-copy{max-width:690px}.open-source-copy p{margin:0 0 16px;color:var(--muted);line-height:1.68}.open-links{display:flex;flex-wrap:wrap;gap:22px;margin-top:28px}.open-links a{font-weight:750;text-decoration:none}
    ...
    @media(max-width:900px){.open-source-strip,.capability-head,.capability-layout{grid-template-columns:1fr}.capability-head{gap:16px}.capability-renders{max-width:680px}.open-source-strip{margin-bottom:72px}}
