...
function tintLayer(variant,key,colour){
  ...
  for(let i=0;i<source.data.length;i+=4){
    if(source.data[i+3]>24){
      sum+=.2126*source.data[i]+.7152*source.data[i+1]+.0722*source.data[i+2];
      count++;
    }
  }
  ...
  for(let i=0;i<output.data.length;i+=4){
    ...
    if(!alpha)continue;
    const lum=.2126*source.data[i]+.7152*source.data[i+1]+.0722*source.data[i+2];
    const delta=(lum-reference)*.72;
  ...
  layer.width=output.width;layer.height=output.height;
  layer.getContext('2d').putImageData(output,0,0);
  tintCache.set(cacheKey,layer);
  return layer;
}

function drawLayer(ctx,layer,width,height,padding=.08){
  const scale=Math.min(width/layer.width,height/layer.height)*(1-padding*2);
  const dw=layer.width*scale,dh=layer.height*scale;
  const dx=(width-dw)/2,dy=(height-dh)/2;
  ctx.drawImage(layer,0,0,layer.width,layer.height,dx,dy,dw,dh);
}

function renderComposite(targetCanvas,colours,variant=selectedVariant,padding=.08){
  const set=layerSets[variant];
  if(!set.images.pcb)return;
  const ctx=targetCanvas.getContext('2d');
  ctx.clearRect(0,0,targetCanvas.width,targetCanvas.height);
  drawLayer(ctx,set.images.pcb,targetCanvas.width,targetCanvas.height,padding);
  drawLayer(ctx,tintLayer(variant,'base',colours.base),targetCanvas.width,targetCanvas.height,padding);
  drawLayer(ctx,tintLayer(variant,'lid',colours.lid),targetCanvas.width,targetCanvas.height,padding);
  drawLayer(ctx,tintLayer(variant,'button1',colours.button1),targetCanvas.width,targetCanvas.height,padding);
  drawLayer(ctx,tintLayer(variant,'button2',colours.button2),targetCanvas.width,targetCanvas.height,padding);
  drawLayer(ctx,set.images.lightPipes,targetCanvas.width,targetCanvas.height,padding);
}
