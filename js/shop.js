...
realPreviewStyles.href = 'css/shop-real.css?v=20260820-radar2';
...
const layerPaths={
  ...
  presence:{base:'images/radar-base.png',pcb:'images/radar-pcb.png',lid:'images/radar-lid.png',lightPipes:'images/radar-lightPipes.png',button1:'images/radar-button1.png',button2:'images/radar-button2.png'}
};
const layerSets={compact:{images:{},sourceData:{},bounds:{}},presence:{images:{},sourceData:{},bounds:{}}};
...
function loadImage(variant,key,src){return new Promise((resolve,reject)=>{const image=new Image();image.onload=()=>{const set=layerSets[variant];set.images[key]=image;const off=document.createElement('canvas');off.width=canvas.width;off.height=canvas.height;const ctx=off.getContext('2d',{willReadFrequently:true});ctx.drawImage(image,0,0,canvas.width,canvas.height);set.sourceData[key]=ctx.getImageData(0,0,canvas.width,canvas.height);resolve()};image.onerror=reject;image.src=src})}

// Each exported PNG has its own transparent crop. Work out bounds PER LAYER,
// rather than using one combined crop for every image.
function calculateBounds(source){let minX=source.width,minY=source.height,maxX=-1,maxY=-1;for(let y=0;y<source.height;y+=2){for(let x=0;x<source.width;x+=2){if(source.data[(y*source.width+x)*4+3]>10){minX=Math.min(minX,x);minY=Math.min(minY,y);maxX=Math.max(maxX,x);maxY=Math.max(maxY,y)}}}if(maxX<minX||maxY<minY)return{x:0,y:0,width:source.width,height:source.height};return{x:minX,y:minY,width:maxX-minX+1,height:maxY-minY+1}}
...
// The PNGs were rendered from the same camera and canvas, so they must be drawn
// using the full source canvas. Cropping individual layers would destroy alignment.
function drawLayer(ctx,layer,width,height,padding=.08){const scale=Math.min(width/layer.width,height/layer.height)*(1-padding*2);const dw=layer.width*scale,dh=layer.height*scale,dx=(width-dw)/2,dy=(height-dh)/2;ctx.drawImage(layer,0,0,layer.width,layer.height,dx,dy,dw,dh)}
...
Promise.all(loadPromises).then(()=>{Object.keys(layerSets).forEach(variant=>{Object.entries(layerSets[variant].sourceData).forEach(([key,source])=>layerSets[variant].bounds[key]=calculateBounds(source))});renderScoopy();renderFlavourPreviews()}).catch(error=>console.error('Unable to load Scoopy preview layers',error));
